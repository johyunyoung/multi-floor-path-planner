...
void laserCloudHandler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr laserCloud2)
{
  if (!useTerrainAnalysis) {
    ...
    for (int i = 0; i < laserCloudSize; i++) {
      ...
      float cosYaw = cos(vehicleYaw);
      float sinYaw = sin(vehicleYaw);
      float pointX = cosYaw * point.x - sinYaw * point.y + vehicleX;
      float pointY = sinYaw * point.x + cosYaw * point.y + vehicleY;
...
int main(int argc, char** argv)
{
  ...
  auto subOdometry = nh->create_subscription<nav_msgs::msg::Odometry>("/state_estimation", rclcpp::SensorDataQoS(), odometryHandler);

  auto subLaserCloud = nh->create_subscription<sensor_msgs::msg::PointCloud2>("/registered_scan", rclcpp::SensorDataQoS(), laserCloudHandler);

  auto subTerrainCloud = nh->create_subscription<sensor_msgs::msg::PointCloud2>("/terrain_map", rclcpp::SensorDataQoS(), terrainCloudHandler);
